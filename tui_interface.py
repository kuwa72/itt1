...
class iTunesTUI:
    ...
    def __init__(self, config_manager: ConfigManager = None, use_global_hook: bool = True):
        ...
        self.key_bindings = {
            ...
            'q': self.quit,
            'r': self.refresh_playlists,
            'c': self.create_config_playlists,
            'p': self.pick_quick_slots
        }
        
        # プレイリスト追加用のキーバインディング（常に1-5をバインド）
        for i in range(5):
            self.key_bindings[str(i + 1)] = (lambda idx=i: (lambda: self.add_to_playlist(idx)))()
    ...
    def add_to_playlist(self, playlist_index: int):
        ...
        if 0 <= playlist_index < len(self.config_playlists):
    ...
    def pick_quick_slots(self):
        """iTunes上の既存プレイリストからクイックスロット(1-5)を選択"""
        ...
        self.console.print(Panel(table, title="クイックスロットに割り当てるプレイリスト番号をカンマ区切りで最大5つ入力", border_style="magenta"))
        try:
            ...
            for i in indices:
                if i not in unique:
                    unique.append(i)
            new_slots = [all_names[i] for i in unique[:5]]
            if new_slots:
                self.config_manager.set_quick_slots(new_slots)
                self.config_playlists = new_slots
                # 1-5のバインディングを再構築
                for k in list(self.key_bindings.keys()):
                    if k.isdigit() and 1 <= int(k) <= 9:
                        self.key_bindings.pop(k, None)
                for i in range(5):
                    self.key_bindings[str(i + 1)] = (lambda idx=i: (lambda: self.add_to_playlist(idx)))()
    ...
    def get_track_display(self) -> Text:
        ...
        if not self.config_playlists:
            text.append("\n[ヒント] 'p' でクイックスロット（1-5）にプレイリストを割り当てできます", style="yellow")
    ...
    def get_playlists_display(self) -> Table:
        ...
        for i, playlist_name in enumerate(self.config_playlists, 1):
            status = "存在" if any(p['name'] == playlist_name for p in self.playlists) else "未作成"
            style = "green" if status == "存在" else "red"
            table.add_row(str(i), playlist_name, Text(status, style=style))
    ...
    def get_help_display(self) -> Text:
        ...
        text.append("1-5: クイックスロット1-5に追加\n")
        
        text.append("その他:\n", style="bold yellow")
        text.append("r: プレイリスト更新  ")
        text.append("c: プレイリスト作成  ")
        text.append("p: クイックスロット選択  ")
        text.append("q: 終了")
    ...
    def run(self):
        ...
        if self.use_global_hook:
            def on_key_press(event):
                if event.name in self.key_bindings:
